...
RgbdSlamNode::RgbdSlamNode(const std::shared_ptr<PLPSLAM::config>& cfg,
                           ...
                           bool mapping)
    : Node("PLPSLAM_RGBD"), eval_log_(eval_log), map_db_path_(map_db_path)
{
    ...
    sub1_ = std::make_shared<message_filters::Subscriber<ImageMsg>>(this, "camera/rgb");
    sub2_ = std::make_shared<message_filters::Subscriber<ImageMsg>>(this, "camera/depth");
    sync_ = std::make_shared<message_filters::Synchronizer<approximate_sync_policy>>(
        approximate_sync_policy(10), *sub1_, *sub2_);
    sync_->registerCallback(&RgbdSlamNode::GrabFrame, this);

    RCLCPP_INFO(get_logger(), "PLP-SLAM Rgbd node started");
}
...
RgbdSlamNode::~RgbdSlamNode()
{
    ...
    m_SLAM->shutdown();
    RCLCPP_INFO(get_logger(), "PLP-SLAM Rgbd node shutdown");
}

void RgbdSlamNode::GrabFrame(const ImageMsg::SharedPtr msg1, const ImageMsg::SharedPtr msg2)
{
    auto tp_1 = std::chrono::steady_clock::now();
    try {
        auto cv_ptr1 = cv_bridge::toCvShare(msg1, "bgr8");
        auto cv_ptr2 = cv_bridge::toCvShare(msg2);
        m_SLAM->feed_RGBD_frame(cv_ptr1->image, cv_ptr2->image, Utility::StampToSec(msg1->header.stamp));
