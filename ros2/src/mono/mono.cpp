...
int main(int argc, char** argv)
{
    if (argc < 3)
    {
        std::cerr << "Usage: ros2 run plpslam_ros2 mono path_to_vocab path_to_config [use_line_tracking]" << std::endl;
        return 1;
    }
    ...
    auto node = std::make_shared<MonoSlamNode>(cfg, vocab_path, use_line);
