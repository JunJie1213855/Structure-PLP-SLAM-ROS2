...
MonoSlamNode::MonoSlamNode(const std::shared_ptr<PLPSLAM::config>& cfg,
                           ...
                           bool mapping)
    : Node("PLPSLAM_Mono"), eval_log_(eval_log), map_db_path_(map_db_path)
{
    ...
    if (!load_map_path.empty())
    {
        ...
        m_SLAM->startup(false);  // don't initialize a new map
        if (mapping) {
    ...
    RCLCPP_INFO(get_logger(), "PLP-SLAM Monocular node started");
}
...
MonoSlamNode::~MonoSlamNode()
{
    ...
    m_SLAM->shutdown();
    RCLCPP_INFO(get_logger(), "PLP-SLAM Monocular node shutdown");
}
